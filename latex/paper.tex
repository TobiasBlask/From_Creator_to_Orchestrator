...
\renewcommand{\headeright}{Position Paper}
\renewcommand{\undertitle}{Position Paper}
...
% [R3-R3: "Related Systems" → "Related Agents" per Burkhardt #1/#3 — consistent "agents" terminology]
\subsection{Related Agents}

Table~\ref{tab:systems} presents autonomous AI research agents that informed the design of the Open Paper Machine. These are the systems that collectively define the landscape from which our pipeline emerged---not competitors, but reference points that clarify shared capabilities (tool-augmented generation, iterative refinement) and design choices specific to our approach (reflexive self-analysis, IS-grounded theory, extensible MCP architecture).

\begin{table}[ht]
\centering
\small
\caption{Autonomous AI research agents.}
\label{tab:systems}
...
% [R4: Three-step evaluation paragraph deleted per Burkhardt — "just delete, ref to tab 1 is also wrong here"]
...
% [R3-R3: §4.2 restructured per Burkhardt #5 — brief phase descriptions added]
...
The six phases are as follows. In \textit{Phase~1 (Reconnaissance)}, the agent executes parallel searches across four academic databases (Semantic Scholar, OpenAlex, CrossRef, arXiv), deduplicates results by DOI, and ranks papers by citation count and topical relevance. In \textit{Phase~2 (Framing)}, it selects theoretical lenses appropriate to the research domain, formulates a research gap, and derives research questions. In \textit{Phase~3 (Structure)}, it builds a concept matrix from the retrieved literature and designs the paper's section hierarchy with word budgets. In \textit{Phase~4 (Production)}, it drafts each section as complete academic prose, integrating citations from the literature base. In \textit{Phase~5 (Assembly)}, it compiles the manuscript into \LaTeX{} and resolves the bibliography. In \textit{Phase~6 (Self-Review)}, it generates a structured self-critique. Table~\ref{tab:quality_gates} specifies the human gate criteria for each phase.
...
% [R3-R3: "Scope and applicability" paragraph moved to §8 (Conclusion) per Burkhardt #4]
...
% [R3-R3: Cross-reference fixed per Burkhardt #7 — was hardcoded "6.1", now §7.1]
Section~\ref{sec:task_redistribution} identified \textit{which} tasks shift; this section asks \textit{how} the human engages with the remaining tasks. \citet{davenport2016just} propose five augmentation strategies. Applied to the orchestrator model:
...
% [R3-R3: Scope paragraph moved here from §4.2 per Burkhardt #4]
